...
class World extends EventTarget {
    ...
    internalStep(dt) {
        ...
        const q = World_step_step_q;
        const w = World_step_step_w;
        const wq = World_step_step_wq;
        const stepnumber = this.stepnumber;
        const DYNAMIC_OR_KINEMATIC = Body.DYNAMIC | Body.KINEMATIC;
        const quatNormalize = stepnumber % (this.quatNormalizeSkip+1) === 0;
        const quatNormalizeFast = this.quatNormalizeFast;
        const half_dt = dt * 0.5;
        const PLANE = Shape.types.PLANE;
        const CONVEX = Shape.types.CONVEXPOLYHEDRON;
        ...
        for(i=0; i!==N; i++){
            ...
            if((b.type & DYNAMIC_OR_KINEMATIC) && b.sleepState !== Body.SLEEPING){
                // Only for dynamic
                const velo = b.velocity;

                const angularVelo = b.angularVelocity;
                const pos = b.position;
                const quat = b.quaternion;
                const invMass = b.invMass;
                const invInertia = b.invInertiaWorld;
 ...
 * @param {Number} dt
 */
var // Reusable event objects to save memory
...
var // Pools for unused objects
World_step_oldContacts = [];

var World_step_frictionEquationPool = [];

var // Reusable arrays for collision pairs
World_step_p1 = [];

var World_step_p2 = [];

const // Temporary vectors and quats
